...
def test_ceiling_uses_the_prescribed_range_not_the_shifted_one():
    # Находка 1 ревью Задачи 12: история записана с диапазоном 8-12
    # (прошлая сессия предписывала именно его), факт 10/10/9 — потолок
    # 8-12 не взят. Персональный сдвиг диапазона (плато -> "сменить режим")
    # сузил ТЕКУЩИЙ диапазон до 5-9. Если бы движок сравнивал вчерашние
    # 10/10/9 с сегодняшним потолком 9, все подходы оказались бы "на
    # потолке" и вес вырос бы — в ответ на решение "стало тяжело, снизим
    # повторы" пользователь получил бы прибавку. Правильное поведение: вес
    # держится, цели считаются от прошлых цифр и упираются в новый rep_max.
    history = last_session(40.0, [10, 10, 9], rep_min=8, rep_max=12)
    p = plan(ctx(history, rep_min=5, rep_max=9))
    assert all(s.weight_kg == pytest.approx(40.0) for s in p.sets), (
        "сдвиг диапазона вниз не должен немедленно повышать вес"
    )
    assert all(s.rep_max == 9 for s in p.sets)

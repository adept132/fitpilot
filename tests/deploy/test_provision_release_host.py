...
if command == "install":
    ...
    mounts = load(mounts_path); mounts.pop(key(args[-1]), None); save(mounts_path, mounts)
elif command == "ln":
    target, link = args[-2:]
    ...
    action = next((a.split("=", 1)[1] for i, a in enumerate(args[:-1]) if a == "-e" and args[i + 1].startswith("EURITH_PROBE_ACTION=") for a in [args[i + 1]]), "")
    final_dir = pathlib.Path(os.environ["FAKE_FINAL_DIR"])
    staging_dir = pathlib.Path(os.environ["FAKE_STAGING_DIR"])
    fixture = final_dir / ("f" * 64 + ".apk")
    if action == "api-stage":
        if "printf probe" not in joined or "chmod 0600" not in joined or "mv -T" not in joined or "chmod 0640" not in joined: sys.exit(3)
        staging = staging_dir / ("f" * 64 + ".tmp")
        staging.write_bytes(b"probe"); os.chmod(staging, 0o600)
        fixture.parent.mkdir(parents=True, exist_ok=True); staging.replace(fixture); os.chmod(fixture, 0o640)
        sys.exit(0)
    if action == "api-remove":
        fixture.unlink(missing_ok=True); sys.exit(0)
    ...
    if action == "caddy-read": sys.exit(0 if fixture.is_file() else 1)
...
def _make_fake_commands(bin_dir: Path, state_dir: Path) -> None:
    ...
    for command in ("docker", "findmnt", "mount", "umount", "getent", "groupadd", "install", "chown", "stat", "ln", "readlink", "head", "mv"):

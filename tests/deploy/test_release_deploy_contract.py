...
def test_dirty_checkout_stops_before_any_mutating_command(tmp_path: Path) -> None:
    ...
    for command in ("docker", "curl", "sha256sum", "awk", "sed", "grep", "findmnt", "head", "python3"):
...
def test_failed_negative_auth_canary_stops_before_container_checks(tmp_path: Path) -> None:
    ...
    driver.write_text(
        ...
        "h.write_text('Cache-Control: no-store\\n'); b.write_text('{\"database\":\"connected\"}')\n"
    ...
    _wrapper(bin_dir, "stat", 'if [[ "$*" == *"%a:%u"* ]]; then printf "400:0\\n"; else exec /usr/bin/stat "$@"; fi')

...
Use this single production deployment entry point with the exact reviewed
commit SHA (not a floating branch name):
...
- The server checkout is clean and the exact SHA resolves after the script's
  fetch.
- There is enough free disk space for a PostgreSQL backup and the target image.
- The pre-deploy backup created by the script is non-empty; record its
  SHA-256 checksum and its path without exposing database credentials.
...
The script performs this ordered sequence: verify a clean worktree, fetch and
resolve the exact target SHA, create a PostgreSQL backup, check out the target,
build its API image, apply migrations, apply and check the reviewed localization
catalog using that newly built image, restart the API, then poll the public
health endpoint.
...
old API untouched. It prints the backup path. Additive migration columns and
catalog writes can remain in the database; this is expected rollback scope and
does not mean the database was restored.

If failure happens after the API restart (including public health failure), the
script restores the old source checkout, rebuilds the old image, and restarts
the old API. It does not perform an automatic schema downgrade or automatic
database restore. Escalate to the backup owner before any manual restore or
downgrade decision.

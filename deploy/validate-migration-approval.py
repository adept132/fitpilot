...
def validate_payload(metadata: os.stat_result, raw: bytes, old_sha: str, target_sha: str, old_head: str, target_head: str, path_hash: str) -> tuple[str, str]:
    ...
    if not SHA40.fullmatch(old_sha) or not SHA40.fullmatch(target_sha):
    ...
    if lines[:6] != prefix or not lines[6].startswith("approval_identity="):
        raise ValueError("approval_mismatch")
    identity = lines[6].split("=", 1)[1]
    if not IDENTITY.fullmatch(identity):
        raise ValueError("approval_identity_invalid")
    return hashlib.sha256(raw).hexdigest(), identity


def validate(path: Path, old_sha: str, target_sha: str, old_head: str, target_head: str, path_hash: str) -> tuple[str, str]:
    ...
    try:
        ...
        os.close(descriptor)
    return validate_payload(metadata, raw, old_sha, target_sha, old_head, target_head, path_hash)

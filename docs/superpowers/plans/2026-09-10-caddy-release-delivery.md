...
- As hardened by the 2026-09-12 boot-persistence revision, storage root and `android` are root-owned setgid mode `2750`; only `.staging` and final `sha256` leaves retain API-UID-owned `2770`. All source/target ancestors are root-controlled without group/other write. Finalized APKs are mode `0640`, and runtime UID/GID values are resolved rather than assumed.

...
def plan(ctx: SchemeContext) -> Prescription:
    ...
    if weight is not None:
        # Находка 2 ревью Задачи 12 (P0-06): anchor берётся из истории/
        # состояния как есть и сеткой шага оборудования не связан (реальный
        # вес пользователя мог быть залогирован до смены шага, вручную,
        # из импорта). Ветки этой функции, которые НЕ проходят через явный
        # round_to_step ниже (нет истории; хотя бы один подход ниже
        # потолка), обязаны получить вес, гарантированно достижимый на
        # сетке — иначе предписание просит набрать вес, которого нет.
        weight = round_to_step(
            weight, list(ctx.equipment), ctx.unit, ctx.weight_steps or None
        )

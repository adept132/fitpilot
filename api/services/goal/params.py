...
# Причины без единого рычага, которые ВСЁ РАВНО заслуживают предложения
# (Задача 17, §5.4 решение 13): рычагов нет, но честный ответ — «сдвиньте
# срок» — есть и достоин карточки на экране, а не тишины. REASON_TREND_DOWN
# сюда НЕ входит: спека прямо отдаёт падающий тренд решениям P0-07/P0-08
# ("на падающем тренде автопилот не ускоряется"), и сдвиг дедлайна тренд не
# лечит — предлагать его здесь означало бы советовать не то.
LEVERLESS_PROPOSAL_REASONS = frozenset({REASON_ABOVE_CEILING, REASON_NO_LEVER_LEFT})

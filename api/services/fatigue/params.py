...
PARAM_REGISTRY: dict[str, Param] = {
    ...
    "sigma_floor_ratio": Param(
        ...
        # РАСХОЖДЕНИЕ СО СПЕКОЙ: §4.3 задаёт пол как долю от МЕДИАНЫ F_c, а
        # readiness_z сейчас берёт долю от abs(среднего). На регулярных данных
        # разница копеечная, но медиана устойчивее к выбросам. Оставлено на
        # решение (см. открытые вопросы в леджере) — здесь описано реальное
        # поведение кода, а не спека.
        "Пол дисперсии как доля от среднего |F_c|: у очень регулярного "
        "пользователя std стремится к нулю и z взрывается на копейках (§4.3).",
    ),

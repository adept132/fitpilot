...
def clamp_target(raw_target: float, lm: Landmarks, cycle_multiplier: float) -> int:
    """Загнать сырую цель в физиологический диапазон мышцы.

    Границы масштабируются тем же множителем, что и цель: при
    десятидневном микроцикле законный объём окна выше недельного, и
    неотмасштабированный потолок срезал бы его как превышение.

    Обе границы округляются ВНИЗ (тот же принцип, что у mev_direct/
    mrv_direct в landmarks.landmarks_for): пол мягче, потолок строже —
    и это единообразие проверяет test_clamp_scales_bounds_by_cycle_multiplier.
    Изначально здесь стоял math.ceil для потолка — при cycle_multiplier
    10/7 он давал 26 вместо ожидаемых тестом 25; заменено на math.floor.
    """
    low = math.floor(lm.mev * cycle_multiplier)
    high = math.floor(lm.mrv * cycle_multiplier)
    return int(max(low, min(high, math.floor(raw_target))))

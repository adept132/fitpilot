...
async def close_and_advance(
    ...
    *,
    ...
    #
    # P0-08, Задача 10, ревью (задокументировано, НЕ чинить): следующий блок
    # здесь собирается по ПОЛНОМУ снимку фаз закрытого (phase_ops.from_json(
    # block.phases) целиком), а не через phase_ops.split_after — хотя спека
    # Задачи 10 говорила "оставшиеся фазы переезжают в следующий блок". Это
    # осознанное решение, а не недосмотр: close_and_advance — ОБЩАЯ точка для
    # ВСЕХ трёх путей закрытия блока, включая автопереход
    # (roll_over_if_complete) и layoff (_close_for_layoff), для которых полный
    # шаблон — единственно верное поведение (блок закрылся, потому что дошёл
    # до конца по расписанию, а не потому что его укоротили). Если бы здесь
    # использовался split_after, урезанный "хвост" ушёл бы в next_block.phases
    # — а следующий раз, когда УЖЕ ЭТОТ (укороченный) блок сам закроется через
    # roll_over_if_complete, close_and_advance снова возьмёт его текущие
    # phases ЦЕЛИКОМ и передаст их дальше. Усечение увековечило бы само себя:
    # каждый следующий блок навсегда остался бы обрезанным огрызком исходного
    # шаблона, а не полной волной фаз, которую пользователь настраивал.
    # Полный шаблон возвращает волну в исходную форму — это тот инвариант,
    # которым здесь сознательно жертвовать нельзя ради точности одного
    # частного случая (пользовательское close_block).

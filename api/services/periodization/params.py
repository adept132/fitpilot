...
# P0-09: обзор объёма за закрытый микроцикл. Живёт в этой же таблице, а не
# в своей: рельса решений (идемпотентность, офлайн-очередь, карточка,
# статус expired) уже построена, а вторая такая была бы дублированием.
# "periodization" в имени таблицы с этого момента значит "решения движка",
# а не только фазы.
KIND_VOLUME_REVIEW = "volume_review"
...
# --- Окно снимка состояния на входе в блок ---
# Снимок на входе в блок собирается по недавним упражнениям, а не по всей
# истории: на каждое упражнение приходится отдельная загрузка истории, а
# смысл снимка — зафиксировать состояние актуальных движений, а не тех,
# что человек делал год назад.
...
# Сколько блоков подряд можно перекатить за один вызов. Человек, вернувшийся
# после долгого перерыва, не должен получить пустой календарь, но и
# восстанавливать цепочку из десятков блоков бессмысленно: дальше этого
# предела считаем, что прежняя программа потеряла смысл, и начинаем блок с
# сегодня.
MAX_CHAIN_ROLLOVERS = 6
